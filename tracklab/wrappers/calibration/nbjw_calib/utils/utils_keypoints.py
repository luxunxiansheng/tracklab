...
class KeypointsDB(object):
    ...
    def __init__(self, data, image):
        ...
        self.keypoints3_check_dict = {
            "Big rect. left main": [46],
            "Circle left": [47],
            "Middle line": [51],
        ...
        self.keypoint_aux_world_coords_2D = [
            [5.5, 0],
            [16.5, 0],
            [88.5, 0],
            [99.5, 0],
    ...
    def get_full_keypoints(self):
        ...
        self.refine_sanity_check(
            [self.keypoints, self.keypoints_aux], [self.keypoints3]
        )
            ...
            if self.keypoints[kp]:
                if self.keypoints[kp]["close_to_frame"]:
                    ...
                    ax.scatter(
                        x,
                        y,
                        c="orange" if self.keypoints[kp]["retrieved"] else "r",
                        s=scale * 10,
                        zorder=10,
                    )
            ...
            if self.keypoints1[kp] and kp in list(range(31, 37)):
                if self.keypoints1[kp]["close_to_frame"]:
                    ...
                    if "proj_err" in self.keypoints1[kp].keys():
                        if self.keypoints1[kp]["proj_err"] > 5.0:
            ...
            if self.keypoints2[kp] and kp in list(range(37, 45)):
                if self.keypoints2[kp]["close_to_frame"]:
                    ...
                    if "proj_err" in self.keypoints2[kp].keys():
                        if self.keypoints2[kp]["proj_err"] > 5.0:
                            ax.scatter(x, y, c="black", s=scale * 10, zorder=10)
                        else:
        ...
        for line in ["Circle left", "Circle central", "Circle right"]:
            if line in self.data.keys():
                if len(self.data[line]) == 4:
                    ...
                    y_upsample = poly_func(x_upsample) + np.random.normal(
                        scale=0.5, size=num_upsample_points
                    )

                    points = [
                        {"x": p[0] / self.w, "y": p[1] / self.h}
                        for p in list(zip(x_upsample, y_upsample))
                    ]
    ...
    def get_correspondences(
        ...
        if keypoints:
            for kp in self.keypoints.keys():
                if (
                    self.keypoints[kp]["close_to_frame"]
                    and not self.keypoints[kp]["retrieved"]
                ):
                    wp = self.keypoint_world_coords_2D[kp - 1]
                    if kp in [12, 16]:
                        world_points_p2.append([2.44, wp[1], 0.0])
                        img_points_p2.append(
                            [self.keypoints[kp]["x"], self.keypoints[kp]["y"]]
                        )
                    elif kp in [1, 4, 8, 13, 17, 20, 24, 28]:
                        world_points_p1.append([wp[0], wp[1], 0.0])
                        world_points_p2.append([0.0, wp[1], 0.0])
                        ...
                        world_points_p1.append([wp[0], wp[1], 0.0])
                        world_points_p3.append([0.0, wp[1], 0.0])
                        img_points_p1.append(
                            [self.keypoints[kp]["x"], self.keypoints[kp]["y"]]
                        )
                        ...
                        world_points_p3.append([2.44, wp[1], 0.0])
                        img_points_p3.append(
                            [self.keypoints[kp]["x"], self.keypoints[kp]["y"]]
                        )
                    else:
        ...
        if keypoints_aux:
            for kp in self.keypoints_aux.keys():
                if self.keypoints_aux[kp]["close_to_frame"]:
                    ...
                    world_points_p1.append([wp[0], wp[1], 0.0])
                    img_points_p1.append(
                        [self.keypoints_aux[kp]["x"], self.keypoints_aux[kp]["y"]]
                    )
        ...
        if keypoints1:
            for kp in self.keypoints1.keys():
                if self.keypoints1[kp]["close_to_frame"]:
                    ...
                    world_points_p1.append([wp[0], wp[1], 0.0])
                    img_points_p1.append(
                        [self.keypoints1[kp]["x"], self.keypoints1[kp]["y"]]
                    )
        ...
        if keypoints2:
            for kp in self.keypoints2.keys():
                if self.keypoints2[kp]["close_to_frame"]:
                    ...
                    world_points_p1.append([wp[0], wp[1], 0.0])
                    img_points_p1.append(
                        [self.keypoints2[kp]["x"], self.keypoints2[kp]["y"]]
                    )
        ...
        if keypoints3:
            for kp in self.keypoints3.keys():
                if self.keypoints3[kp]["close_to_frame"]:
                    ...
                    world_points_p1.append([wp[0], wp[1], 0.0])
                    img_points_p1.append(
                        [self.keypoints3[kp]["x"], self.keypoints3[kp]["y"]]
                    )

        if only_ground_plane:
            return [world_points_p1], [img_points_p1]
        else:
            return [world_points_p1, world_points_p2, world_points_p3], [
                img_points_p1,
                img_points_p2,
                img_points_p3,
            ]
    ...
    def get_frame_projection(self, obj_list, img_list, use_ransac=25.0):
        ...
        for i in range(len(obj_list)):
            if len(obj_list[i]) >= 4 and not all(
                item[0] == obj_list[i][0][0] for item in obj_list[i]
            ):
        ...
        if use_ransac > 0:
            H, mask = cv2.findHomography(
                np.array(obj_points[0], dtype=np.float32),
                np.array(img_points[0], dtype=np.float32),
                cv2.RANSAC,
                use_ransac,
            )
        else:
    ...
    def refine_sanity_check(self, set_reference, set_to_check):
        ...
        def find_first_close_to_frame(dictionary):
            for key, nested_dict in dictionary.items():
                if nested_dict.get("close_to_frame") == True and key not in [
                    12,
                    15,
                    16,
                    19,
        ...
        def find_columns_with_minus_one(matrix):
            return [
                ...
                if any(row[col] == -1 for row in matrix)
            ]
        ...
        if ref_point1:
            ...
            for count1, kp1 in enumerate(dict_ref.keys()):
                if kp1 not in [12, 15, 16, 19]:
                    wp1 = (
                        self.keypoint_world_coords_2D[kp1 - 1]
                        if kp1 < 57
                        else self.keypoint_aux_world_coords_2D[kp1 - 57 - 1]
                    )
                    vw1 = np.array([wp1[0], wp1[1]]) - np.array([wpr[0], wpr[1]])
                    vi1 = np.array([dict_ref[kp1]["x"], dict_ref[kp1]["y"]]) - np.array(
                        [ipr[0], ipr[1]]
                    )
                    for count2, kp2 in enumerate(dict_check.keys()):
                        wp2 = self.keypoint_world_coords_2D[kp2 - 1]
                        is_collinear = are_points_collinear(wpr, wp1, wp2, 5.0)
                        ...
                        if is_collinear:
                            ...
                            vw2 = np.array([wp2[0], wp2[1]]) - np.array(
                                [wpr[0], wpr[1]]
                            )
                            vi2 = np.array(
                                [dict_check[kp2]["x"], dict_check[kp2]["y"]]
                            ) - np.array([ipr[0], ipr[1]])
            ...
            cols_to_remove = find_columns_with_minus_one(
                np.sign(np.multiply(world_coord_matrix, img_coord_matrix))
            )
    ...
    def retrieve_missing_keypoints(self):
        ...
        obj_list, img_list = self.get_correspondences(
            keypoints1=True, keypoints2=True, only_ground_plane=True
        )
                ...
                if kp not in self.keypoints.keys() and kp not in [12, 15, 16, 19]:
                    ...
                    for kp2 in self.keypoints.keys():
                        kp_dist = np.linalg.norm(
                            np.array(self.keypoint_world_coords_2D[kp2 - 1])
                            - np.array(world_coord)
                        )
                        if kp_dist < min_dist:
                            min_dist = kp_dist
                ...
                if triplet[1] in self.keypoints.keys():
                    if self.keypoints[triplet[1]]["close_to_frame"]:
                        ...
                        if len(x1) > 4:
                            ...
                            ext_p = np.array(
                                [
                                    self.keypoints[triplet[1]]["x"],
                                    self.keypoints[triplet[1]]["y"],
                                ]
                            )
                            inter_list = find_tangent_points(
                                center, width, height, theta, ext_p
                            )
    ...
    def get_keypoints1and2(self):
            ...
            if H is not None:
                ...
                for count, point in enumerate(obj_list_aux[0]):
                    ...
                    rep_err += np.sum(
                        (
                            np.array(
                                [img_list_aux[0][count][0], img_list_aux[0][count][1]]
                            )
                            - p[:2]
                        )
                        ** 2
        ...
        def sanity_check(candidates_set, combination):
            ...
            imr = [
                self.keypoints[reference_point]["x"],
                self.keypoints[reference_point]["y"],
            ]
            ...
            for count1, kp1 in enumerate(candidates_set):
                if combination[count1]:
                    ...
                    vw1 = np.array([wp1[0], wp1[1]]) - np.array([wpr[0], wpr[1]])
                    vi1 = np.array(
                        [combination[count1][0], combination[count1][1]]
                    ) - np.array([imr[0], imr[1]])
                    for count2, kp2 in enumerate(candidates_set):
                        if combination[count2] and count2 != count1:
                            ...
                            vi2 = np.array(
                                [combination[count2][0], combination[count2][1]]
                            ) - np.array([imr[0], imr[1]])
        ...
        if num_proposals != 0:
            ...
            if num_proposals + len(obj_list[0]) >= 4:
                ...
                forbidden_combinations_pos = [
                    (candidates_set.index(p1), candidates_set.index(p2))
                    for p1, p2 in forbidden_combinations
                    if p1 in candidates_set and p2 in candidates_set
                ]
                ...
                combinations = list(
                    product(
                        *[
                            (
                                locations_set1.get(point, [])
                                if point in candidates_set1
                                else locations_set2.get(point, [])
                            )
                            for point in candidates_set1 + candidates_set2
                        ]
                    )
                )
                filtered_combinations = [
                    ...
                    if all(
                        ...
                        or (comb[index1] is None and comb[index2] is None)
                        or (
                            comb[index1] != comb[index2]
                            if comb[index1] is not None and comb[index2] is not None
                            else True
                        )
                ...
                for comb in filtered_combinations:
                    if sanity_check(candidates_set, comb):
                        obj_list_aux, img_list_aux = copy.deepcopy(
                            obj_list
                        ), copy.deepcopy(img_list)
                        for count, kp in enumerate(candidates_set):
                            if comb[count]:
                                wp = self.keypoint_world_coords_2D[kp - 1]
                                obj_list_aux[0].append([wp[0], wp[1], 0.0])
                                img_list_aux[0].append([comb[count][0], comb[count][1]])

                        H = self.get_frame_projection(
                            obj_list_aux, img_list_aux, use_ransac=0
                        )
                        ...
                        if comb_rep_err < min_rep_err_sub and any(
                            x is not None for x in comb
                        ):
                ...
                final_comb = (
                    min_rep_err_comb
                    if min_rep_err_comb
                    else min_rep_err_sub_comb if min_rep_err_sub_comb else None
                )
                    ...
                    for count1, kp1 in enumerate(candidates_set1):
                        ...
                        if final_comb[count1]:
                            ...
                            self.keypoints1[kp1] = {
                                ...
                                "close_to_frame": -self.w_extra
                                <= x
                                <= self.w + self.w_extra
                                and -self.h_extra <= y <= self.h + self.h_extra,
                            }
    ...
    def get_keypoints1_from_ellipse(self):
        ...
        mask_ellipse_intersection = {
            "left": [["Circle left", "Big rect. left main"], [31, 34]],
        ...
        if not all(value is None for value in self.keypoints.values()):
            ...
            if H is not None:
                ...
                for count, pair in enumerate(self.keypoint1_pair_list):
                    if all(x in self.data.keys() for x in pair):
                        intersections = ellipse_intersection(
                            self.data, pair, self.w, self.h
                        )
                        if len(intersections) == 2:
                            ...
                            self.keypoints1[count + 30 + 1] = {
                                ...
                                "in_frame": (
                                    0 <= intersections[matching_sort[0][0]][0] < self.w
                                    and 0
                                    <= intersections[matching_sort[0][0]][1]
                                    < self.h
                                ),
                                "close_to_frame": (
                                    0 - self.w_extra
                                    <= intersections[matching_sort[0][0]][0]
                                    < self.w + self.w_extra
                                    and 0 - self.h_extra
                                    <= intersections[matching_sort[0][0]][1]
                                    < self.h + self.h_extra
                            ...
                            self.keypoints1[count + 30 + 4] = {
                                ...
                                "in_frame": (
                                    0 <= intersections[matching_sort[1][0]][0] < self.w
                                    and 0
                                    <= intersections[matching_sort[1][0]][1]
                                    < self.h
                                ),
                                "close_to_frame": (
                                    0 - self.w_extra
                                    <= intersections[matching_sort[1][0]][0]
                                    < self.w + self.w_extra
                                    and 0 - self.h_extra
                                    <= intersections[matching_sort[1][0]][1]
                                    < self.h + self.h_extra
                            ...
                            if matching[0][1] == 0:
                                ...
                                if (
                                    self.keypoints1[count + 30 + 1]["proj_err"]
                                    > self.proj_err_th
                                ):
                                    self.mask_array[count + 30] = 0
                                ...
                                if (
                                    self.keypoints1[count + 30 + 4]["proj_err"]
                                    > self.proj_err_th
                                ):
                                    self.mask_array[count + 30 + 3] = 0
                        else:
                ...
                for side in mask_ellipse_intersection.keys():
                    if all(
                        x in self.data.keys()
                        for x in mask_ellipse_intersection[side][0]
                    ):
    ...
    def get_keypoints2_from_tangents(self):
        ...
        if not all(value is None for value in self.keypoints.values()):
            ...
            obj_list, img_list = self.get_correspondences(
                only_ground_plane=True, keypoints1=True
            )
            H = self.get_frame_projection(obj_list, img_list)
            if H is not None:
                        ...
                        if triplet[1] in self.keypoints.keys():
                            if self.keypoints[triplet[1]]["close_to_frame"]:
                                ...
                                if len(triplet[2]) > 1:
                                    ...
                                    p_world1 = self.keypoint_world_coords_2D[
                                        triplet[2][0] - 1
                                    ]
                                    p_world2 = self.keypoint_world_coords_2D[
                                        triplet[2][1] - 1
                                    ]

                                    p1_proj = np.linalg.inv(H) @ np.array(
                                        [p1[0], p1[1], 1]
                                    )
                                    p1_proj /= p1_proj[-1]
                                    p2_proj = np.linalg.inv(H) @ np.array(
                                        [p2[0], p2[1], 1]
                                    )
                                    p2_proj /= p2_proj[-1]
                                    ...
                                    self.keypoints2[triplet[2][0]] = {
                                        "x": inter_list[matching_sort[0][0]][0],
                                        "y": inter_list[matching_sort[0][0]][1],
                                        "proj_err": cost_matrix[matching_sort[0]],
                                        "in_frame": (
                                            0
                                            <= inter_list[matching_sort[0][0]][0]
                                            < self.w
                                            and 0
                                            <= inter_list[matching_sort[0][0]][1]
                                            < self.h
                                        ),
                                        "close_to_frame": (
                                            0 - self.w_extra
                                    ...
                                    self.keypoints2[triplet[2][1]] = {
                                        "x": inter_list[matching_sort[1][0]][0],
                                        "y": inter_list[matching_sort[1][0]][1],
                                        "proj_err": cost_matrix[matching_sort[1]],
                                        "in_frame": (
                                            0
                                            <= inter_list[matching_sort[1][0]][0]
                                            < self.w
                                            and 0
                                            <= inter_list[matching_sort[1][0]][1]
                                            < self.h
                                        ),
                                        "close_to_frame": (
                                            0 - self.w_extra
                                    ...
                                    if (
                                        self.keypoints2[triplet[2][0]]["proj_err"]
                                        > self.proj_err_th
                                    ):
    ...
    def get_keypoints12strat(self):
        ...
        if n < 4:
            ...
            self.refine_sanity_check(
                [self.keypoints, self.keypoints_aux], [self.keypoints1, self.keypoints2]
            )
    ...
    def get_keypoints3_from_homography(self):
        ...
        def check_num_lines(world_points):
            if len(world_points) > 0:
                ...
                sorted_points_x = projected_points_xy[
                    np.argsort(projected_points_xy[:, 0])
                ]
                sorted_points_y = projected_points_xy[
                    np.argsort(projected_points_xy[:, 1])
                ]
        ...
        penalty_dict = {
            45: [[5, 25], [68, 62]],  # Well paired!
            57: [[6, 26], [69, 63]],
        }

        for kp in [45, 57]:
            if all(
                keypoint in self.keypoints.keys() for keypoint in penalty_dict[kp][0]
            ) and all(
                keypoint in self.keypoints_aux.keys()
                for keypoint in penalty_dict[kp][1]
            ):
                if all(
                    self.keypoints[keypoint]["close_to_frame"] == True
                    for keypoint in penalty_dict[kp][0]
                ) and all(
                    self.keypoints_aux[keypoint]["close_to_frame"] == True
                    for keypoint in penalty_dict[kp][1]
                ):

                    p11 = np.array(
                        [
                            self.keypoints[penalty_dict[kp][0][0]]["x"],
                            self.keypoints[penalty_dict[kp][0][0]]["y"],
                        ]
                    )
                    p12 = np.array(
                        [
                            self.keypoints[penalty_dict[kp][0][1]]["x"],
                            self.keypoints[penalty_dict[kp][0][1]]["y"],
                        ]
                    )
                    p21 = np.array(
                        [
                            self.keypoints_aux[penalty_dict[kp][1][0]]["x"],
                            self.keypoints_aux[penalty_dict[kp][1][0]]["y"],
                        ]
                    )
                    p22 = np.array(
                        [
                            self.keypoints_aux[penalty_dict[kp][1][1]]["x"],
                            self.keypoints_aux[penalty_dict[kp][1][1]]["y"],
                        ]
                    )
                    ...
                    self.keypoints3[kp] = {
                        ...
                        "y": y_intersection,
                        "in_frame": (
                            0 <= x_intersection < self.w
                            and 0 <= y_intersection < self.h
                        ),
                        "close_to_frame": (
                            0 - self.w_extra <= x_intersection < self.w + self.w_extra
                            and 0 - self.h_extra
                            <= y_intersection
                            < self.h + self.h_extra
                        ),
        ...
        obj_list, img_list = self.get_correspondences(
            only_ground_plane=True, keypoints1=True, keypoints2=True, keypoints3=True
        )
            ...
            if H is not None:
                ...
                for kp in points_to_compute:
                    ...
                    self.keypoints3[kp] = {
                        ...
                        "close_to_frame": (
                            0 - self.w_extra <= p[0] < self.w + self.w_extra
                            and 0 - self.h_extra <= p[1] < self.h + self.h_extra
                        ),
                    }
