...
class LineKeypointsDB(object):
    ...
    def get_tensor(self):
        ...
        heatmap_tensor = generate_gaussian_array_vectorized_l(
            self.num_channels, self.lines, self.size, down_ratio=2, sigma=2
        )
    ...
    def draw_keypoints(self, show_heatmap=False):
        ...
        if show_heatmap:
            heatmap = generate_gaussian_array_vectorized_l(
                self.num_channels, self.lines, self.size, down_ratio=2, sigma=2
            )
    ...
    def find_most_distanced_points(self, segment):
        ...
        distances = [
            math.dist((pair[0]["x"], pair[0]["y"]), (pair[1]["x"], pair[1]["y"]))
        ...
        for line in self.data.keys():
            if line in self.lines_list:
                if len(self.data[line]) > 1:
                    ...
                    self.lines[self.lines_list.index(line) + 1] = {
                        "x_1": x1 * self.w,
                        "y_1": y1 * self.h,
                        "x_2": x2 * self.w,
                        "y_2": y2 * self.h,
    ...
    def refine_point_lines(self):
        ...
        for line in self.lines.keys():
            ...
            kp1 = (
                np.array(
                    [
                        self.keypoints[kp_to_refine[0]]["x"],
                        self.keypoints[kp_to_refine[0]]["y"],
                    ]
                )
                if self.keypoints[kp_to_refine[0]]["in_frame"]
                else np.array([np.nan, np.nan])
            )
            kp2 = (
                np.array(
                    [
                        self.keypoints[kp_to_refine[1]]["x"],
                        self.keypoints[kp_to_refine[1]]["y"],
                    ]
                )
                if self.keypoints[kp_to_refine[1]]["in_frame"]
                else np.array([np.nan, np.nan])
            )
