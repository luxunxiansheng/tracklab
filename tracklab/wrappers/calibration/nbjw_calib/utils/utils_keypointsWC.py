...
class KeypointsWCDB(object):
    def __init__(self, image, homography, size_out=(960, 540)):
        ...
        self.lines_retrieval = [
            ...
            [2, 29],
            [28, 29, 30],
            [1, 4, 8, 13, 17, 20, 24, 28],
            [3, 7, 11, 14, 18, 23, 27, 30],
            [1, 2, 3],
            [20, 21],
            [9, 21],
            [8, 9],
    ...
    def get_tensor_w_mask(self):
        ...
        heatmap_tensor = generate_gaussian_array_vectorized(
            self.num_channels, self.keypoints_final, self.size, down_ratio=2, sigma=2
        )
        return heatmap_tensor, self.mask_array

    def kpmeters2yards(self, kp):
        wp = self.keypoint_world_coords_2D[kp - 1]
        wp_arr = np.array([wp[0] * 1.09361, wp[1] * 1.09361, 1.0])
        return wp_arr
    ...
    def get_kp_from_homography(self):
        for kp in range(1, len(self.keypoint_world_coords_2D) + 1):
            if kp not in [12, 15, 16, 19]:
                ...
                wp = self.keypoint_world_coords_2D[kp - 1]
                img_pt = np.linalg.inv(self.homography) @ np.array([wp[0], wp[1], 1.0])
                img_pt /= img_pt[-1]
                img_pt[0] *= self.w / self.image.size[0]
                img_pt[1] *= self.h / self.image.size[1]

                self.keypoints_final[kp] = {
                    "x": img_pt[0],
                    "y": img_pt[1],
                    "in_frame": (
                        True
                        if 0 <= img_pt[0] <= self.w and 0 <= img_pt[1] <= self.w
                        else False
                    ),
                    "close_to_frame": (
                        True
                        if -self.w_extra <= img_pt[0] <= self.w + self.w_extra
                        and -self.h_extra <= img_pt[1] <= self.h + self.h_extra
                        else False
                    ),
                }
