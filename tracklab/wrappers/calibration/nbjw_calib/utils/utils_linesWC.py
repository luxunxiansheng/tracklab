...
class LineKeypointsWCDB(object):
    ...
    def get_lines(self):
        for count, line in enumerate(self.line_extremities):
            if len(line[0]) > 0:
                ...
                p1_img = np.linalg.inv(self.homography) @ np.array(
                    [line[0][0], line[0][1], 1.0]
                )
                p1_img /= p1_img[-1]
                p2_img = np.linalg.inv(self.homography) @ np.array(
                    [line[1][0], line[1][1], 1.0]
                )
                p2_img /= p2_img[-1]
                ...
                flag, pt1, pt2 = cv2.clipLine(
                    (0, 0, self.w, self.h),
                    (int(p1_img[0]), int(p1_img[1])),
                    (int(p2_img[0]), int(p2_img[1])),
                )
    ...
    def kpmeters2yards(self, p):
        wp_arr = np.array([p[0] * 1.09361, p[1] * 1.09361, 1.0])
        return wp_arr
