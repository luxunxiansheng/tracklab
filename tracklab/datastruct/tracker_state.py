...
class TrackerState(AbstractContextManager):
                ...
                if "summary.json" in zf.namelist():
                    with zf.open("summary.json", force_zip64=True) as fp:
                        ...
                        if isinstance(summary["columns"], list):
                            ...
                            load_columns = {
                                k: set(v) for k, v in summary["columns"].items()
                            }
                else:
        ...
        for module in self.pipeline:
            for level in ["image", "detection"]:
                self.input_columns[level] |= (
                    set(module.get_input_columns(level)) - self.output_columns[level]
                )
    ...
    def load_groundtruth(self, load_columns):
        ...
        if self.pipeline.is_empty():
            self.detections_pred_gt = (
                self.detections_gt.copy()
            )  # load all columns if pipeline is empty
            ...
            if isinstance(self.load_from_groundtruth, Mapping):
                if "detection" in self.load_from_groundtruth:
                    raise ValueError(
                        "You can't yet load some detections from the detections"
                    )
                load_columns = {
                    k: list(set(self.load_from_groundtruth.get(k, [])) & set(v))
            ...
            if len(load_columns["detection"]) == 0:
                ...
                self.detections_pred_gt = self.detections_gt.copy()[
                    self.detections_gt.columns.intersection(
                        load_columns["detection"] + ["image_id", "video_id"]
                    )
                ]
            self.image_pred_gt = merge_dataframes(
                self.image_metadatas.copy(), self.image_gt.copy()
    ...
    def load_detections_pred_from_json(self, json_file):
        ...
        if self.bbox_format == "ltrb":
            ...
            detections_pred.loc[detections_pred["bbox_ltwh"].notna(), "bbox_ltwh"] = (
    ...
    def update(self, detections: pd.DataFrame, image_metadata):
        if self.detections_pred is None:
            ...
            if self.detections_pred.empty and len(self.detections_pred.columns) == 0:
                self.detections_pred = detections
            else:
            ...
            if self.image_pred.empty and len(self.image_pred.columns) == 0:
                self.image_pred = image_metadata
            else:
    ...
    def load(self):
        ...
        if self.load_from_groundtruth:
            video_detections = self.detections_pred_gt[
                self.detections_pred_gt.video_id == self.video_id
            ]
            video_image_preds = self.image_pred_gt[
                self.image_pred_gt.video_id == self.video_id
            ]
        if self.load_from_public_dets:
            video_detections = self.detections_public[
                self.detections_pred_public.video_id == self.video_id
            ]
            video_image_preds = self.image_pred_public[
                self.image_pred_public.video_id == self.video_id
            ]
        if self.load_file is not None:
            if f"{self.video_id}.pkl" in self.zf["load"].namelist():
                with self.zf["load"].open(
                    f"{self.video_id}.pkl", "r", force_zip64=True
                ) as fp:
                    video_detections = pandas.read_pickle(fp)[
                        self.load_columns["detection"]
                    ]  # TODO see with Victor if this ok
                    video_detections = video_detections[
                        video_detections["image_id"].isin(video_image_preds.index)
                    ]  # load only detections from the required frames (nframes)
            ...
            if f"{self.video_id}_image.pkl" in self.zf["load"].namelist():
                with self.zf["load"].open(
                    f"{self.video_id}_image.pkl", "r", force_zip64=True
                ) as fp_image:
                    video_images = merge_dataframes(
                        pandas.read_pickle(fp_image), video_image_preds
                    )[self.load_columns["image"]]
                    video_image_preds = video_images[
                        video_images.index.isin(video_image_preds.index)
                    ]  # load only images from the required frames (nframes)
            else:
                video_image_preds = self.image_metadatas[
                    self.image_metadatas.video_id == self.video_id
                ]
    ...
    def display_stats(self):
        ...
        if "track_id" in self.detections_pred.columns:
            tracked_text.append(
                f"{len(self.detections_pred.dropna(subset=['track_id']))}"
            )
            unique_text.append(f"{len(self.detections_pred.track_id.unique())}")
        if (
            "track_id" in self.detections_gt.columns
            and "person_id" in self.detections_gt.columns
        ):
            tracked_text.append(
                f" (GT={len(self.detections_gt.dropna(subset=['track_id']))})"
            )
