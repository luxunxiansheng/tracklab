...
def write_video_images_to_disk(video_path: Path) -> Path:
    """Write video frames to disk as individual images.

    Args:
        video_path: Path to the video file.

    Returns:
        Path to the temporary folder containing the images.
    """
...
class ExternalVideo(TrackingDataset):
    ...
    def __init__(self, dataset_path: str, video_path: str, *args, **kwargs) -> None:
        ...
        if video_path.startswith("http"):
            ...
            with yt_dlp.YoutubeDL(yt_params) as ydl:  # type: ignore
        ...
        if self.video_path.is_dir():
            ...
            for i, video_file_path in enumerate(tqdm(list(self.video_path.iterdir()))):
            ...
            video_metadata_df = pd.DataFrame([{"id": video_name, "name": video_name}])
