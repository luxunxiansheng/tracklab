...
from ..ecc import ECC, get_matrix
...
class Tracker:
    ...
    def update(self, detections, classes, confidences):
        ...
        if self.matching_strategy == "strong_sort_matching":
            matches, unmatched_tracks, unmatched_detections = self.strong_sort_matching(
                detections
            )
        elif self.matching_strategy == "bot_sort_matching":
            matches, unmatched_tracks, unmatched_detections = self.bot_sort_matching(
                detections
            )
        else:
            raise NotImplementedError
    ...
    def _full_cost_metric(self, tracks, dets, track_indices, detection_indices):
        ...
        app_cost = self.metric.distance(
            features, targets
        )  # NO thresholding until here -> ONLY REID DISTANCE
        if self.w_reid > 0:
        ...
        st_cost = (
            self.motion_cost(  # return cost matrix gated by KM (too big IOU set to INF)
        ...
        cost_matrix = (
            self.w_kfgd * pos_cost + self.w_reid * app_cost + st_cost * self.w_st
        ) / (self.w_kfgd + self.w_reid + self.w_st)
        if self.w_kfgd > 0:  # FIXME support all combinations
            cost_matrix[np.logical_or(pos_gate, app_gate, st_gate)] = (
    ...
    def strong_sort_matching(self, detections):
        ...
        def gated_metric(tracks, dets, track_indices, detection_indices):
            ...
            targets = np.array([tracks[i].track_id for i in track_indices])
            cost_matrix_reid = self.metric.distance(
                features, targets
            )  # NO thresholding until here -> ONLY REID DISTANCE
        ...
        (matches_b, unmatched_tracks_b, unmatched_detections_b, st_cost_matrix) = (
    ...
    def bot_sort_matching(self, detections):
        ...
        (matches, unmatched_tracks, unmatched_detections, gated_reid_cost_matrix) = (
    ...
    def compute_all_costs_matrix(self, detections):
        ...
        for i, det in enumerate(detections):
            det.costs = {}
            det.costs["R"] = {
                track.track_id: cost_matrix_reid[j, i]
                for j, track in enumerate(self.tracks)
            }
            det.costs["Rt"] = self.metric.matching_threshold
            det.costs["S"] = {
                track.track_id: cost_matrix_st[j, i]
                for j, track in enumerate(self.tracks)
            }
            det.costs["St"] = self.motion_max_distance
            det.costs["K"] = {
                track.track_id: cost_matrix_kf_gating[j, i]
                for j, track in enumerate(self.tracks)
            }
            det.costs["Kt"] = gating_threshold

    def add_matching_information(
        self,
        detections,
        tracks,
        name,
        ...
        for i, d_idx in enumerate(detections_candidates):
            ...
            if d_idx in matches_b_dict:
                matched_dist = cost_matrix[
                    track_glb_idx_to_lcl_idx[matches_b_dict[d_idx]], i
                ]
                det.matched_with = (
                    name,
                    matched_dist,
                )  # name = "S" for Spatio-Temporal or "R" for ReID
            else:
