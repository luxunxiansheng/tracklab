...
class MOT(TrackingDataset):
    def __init__(
        ...
        **kwargs,
        ...
        with Pool(processes=min(3, len(potential_splits))) as pool:
            pool_args = [
                (
                    set_name,
                    self.dataset_path,
                    nvid,
                    vids_dict.get(set_name, []) if vids_dict else [],
                )
                for set_name in potential_splits
            ]
            results = pool.map(self.load_set_wrapper, pool_args)

        sets_dict = {
            set_name: result for set_name, result in results if result is not None
        }
        if leave_one_out_idx is not None:
            if "train" not in sets_dict:
                log.warning(
                    "Leave one out requested but 'train' split not found. Skipping leave one out."
                )
            else:
                video_ids = list(sets_dict["train"].video_metadatas.id)
                loo_video_id = int(
                    sets_dict["train"].video_metadatas.iloc[leave_one_out_idx].id
                )
    ...
    def read_motchallenge_result_formatted_file(self, file_path):
        ...
        if df["bbox_conf"].max() > 1 and df["bbox_conf"].min() < 0:
            log.warning(
                "Warning: 'bbox_conf' from the public detections is not between 0 and 1."
            )
    ...
    def load_set(self, dataset_path, nvid=-1, vids_filter_set=None):
        ...
        split = os.path.basename(
            dataset_path
        )  # Get the split name from the dataset path
        ...
        if nvid > 0:
            assert (
                vids_filter_set is None or len(vids_filter_set) == 0
            ), "Cannot use both 'nvid' and 'vids_dict' arguments at the same time."
        ...
        for video_folder in track(
            sorted(video_list),
            f"Loading {self.__class__.__name__} '{split}' set videos",
        ):  # Sort videos by name
            ...
            if os.path.isdir(video_folder_path):
                ...
                if os.path.isfile(gt_path):
                    ...
                    detections_df["person_id"] = (
                        detections_df["track_id"] + person_counter
                    )
                    detections_df["image_id"] = (
                        detections_df["image_id"] - 1 + image_counter
                    )
                    ...
                    if not warning_trigger_gt:
                        ...
                        log.warning(
                            f"Warning: The {split} split does not contain ground truth."
                        )
                ...
                if self.public_dets_subpath is not None:
                    ...
                    if os.path.isfile(det_path):
                        detections_df = self.read_motchallenge_result_formatted_file(
                            det_path
                        )
                        if detections_df["image_id"].min() == 1:
                            detections_df["image_id"] = detections_df["image_id"] - 1
                        detections_df["image_id"] = (
                            detections_df["image_id"] + image_counter
                        )
                        detections_df["video_id"] = len(video_metadatas_list) + 1
                        ...
                        if not warning_trigger_public_det:
                            ...
                            log.warning(
                                f"Warning: The {split} split does not contain public detections."
                            )
        ...
        if self.public_dets_subpath is not None:
            if len(public_detections_list):
                ...
                public_detections = public_detections.sort_values(
                    by=["video_id", "image_id"], ascending=[True, True]
                )
            else:
        ...
        detections = detections.sort_values(
            by=["video_id", "image_id", "track_id"], ascending=[True, True, True]
        )
        ...
        if vids_filter_set is not None and len(vids_filter_set) > 0:
            ...
            assert (
                not missing_videos
            ), f"The following videos provided in config 'dataset.vids_dict' do not exist in {split} set: {missing_videos}"
            video_list = [video for video in video_list if video in vids_filter_set]
            # get video ids
            video_ids = set(
                video_metadata[video_metadata["name"].isin(video_list)]["id"].tolist()
            )
            ...
            if video_name != "SUMMARIES":
                for category, metrics in video_data["pedestrian"].items():
                    for metric_name, metric_value in metrics.items():
                        ...
                        ):  # Ignore np.array values
                            res_by_vid[
                                f"tracking_by_video/{video_name}/{metric_name}"
                            ] = metric_value
