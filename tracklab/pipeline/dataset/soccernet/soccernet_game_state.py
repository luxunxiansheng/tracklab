...
class SoccerNetGameState(TrackingDataset):
    ...
    def process_trackeval_results(self, results, dataset_config, eval_config):
        ...
        combined_results["GS-HOTA"] = {
            ...
            for k, v in list(combined_results["GS-HOTA"].items())
        }
        log.info(
...
def extract_category(attributes):
    if attributes["role"] == "goalkeeper":
        ...
        if attributes["jersey"] is not None:
            jersey_number = (
                int(attributes["jersey"]) if attributes["jersey"].isdigit() else None
            )
        category = (
            f"{role}_{team}_{jersey_number}"
        ...
        if attributes["jersey"] is not None:
            jersey_number = (
                int(attributes["jersey"])
                if attributes["jersey"].isdigit() is not None
                else None
            )
        category = (
            f"{role}_{team}_{jersey_number}"
...
def dict_to_df_detections(annotation_dict, categories_list):
    ...
    ]  # remove the rows with non-human categories
    ...
    df["position"] = (
        None  # df.apply(lambda row: row['attributes']['position'], axis=1)         for now there is no position in the json file
    )
...
def video_dir_to_dfs(args):
    ...
    if os.path.isdir(video_folder_path):
        if not (Path(video_folder_path) / "Labels-GameState.json").exists():
            ...
            img_metadata_df = pd.DataFrame(
                {
                    ...
                    "id": [
                        f"{split_id}{video_id}{i:06d}" for i in range(1, nframes + 1)
                    ],
                    "video_id": video_id,
                    "file_path": [
                        os.path.join(img_folder_path, f"{i:06d}.jpg")
            ...
            detections_df, annotation_pitch_camera_df, video_level_categories = (
                dict_to_df_detections(annotations_data, categories_data)
            )
            # detections_df['image_id'] = detections_df['image_id'] - 1 + image_counter
            detections_df["video_id"] = video_id
            detections_df["person_id"] = detections_df["track_id"].astype(
                str
            ...
            video_metadata = {
                ...
                "game_time_start": info_data.get("game_time_start", " - ").split(" - ")[
                    1
                ],
                # Remove the half period index
                "game_time_stop": info_data.get("game_time_stop", " - ").split(" - ")[
                    1
                ],  # Remove the half period index
                "clip_stop": int(info_data.get("clip_stop", 0)),
                "num_tracklets": int(info_data.get("num_tracklets", 0)),
                "half_period_start": int(
                    info_data.get("game_time_start", "0 - ").split(" - ")[0]
                ),
                # Add the half period start column
                "half_period_stop": int(
                    info_data.get("game_time_stop", "0 - ").split(" - ")[0]
                ),
                # Add the half period stop column
            }
            # categories_list += video_level_categories
            img_folder_path = os.path.join(
                video_folder_path, info_data.get("im_dir", "img1")
            )
            img_metadata_df = pd.DataFrame(
                {
                    "frame": [i for i in range(0, nframes)],
                    ...
                    "video_id": video_id,
                    "file_path": [
                        os.path.join(img_folder_path, i["file_name"])
...
def load_set(dataset_path, nvid=-1, vids_filter_set=None):
    ...
    if len(categories_list) == 0:
        ...
        pitch_gt = pitch_camera[["image_id", "video_id", "lines"]][
            pitch_camera.supercategory == "pitch"
        ...
        detections["category_id"] = detections["category"].apply(
...
def download_dataset(dataset_path, splits=("train", "valid", "test", "challenge")):
    ...
    if download:
        mySoccerNetDownloader.downloadDataTask(task="gamestate-2025", split=splits)
        for split in splits:
            log.info(f"Unzipping {split} split...")
            with zipfile.ZipFile(
                dataset_path / "gamestate-2025" / f"{split}.zip", "r"
            ) as zf:
