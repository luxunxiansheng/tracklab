...
class SoccerNetMOT(TrackingDataset):
    def __init__(
        ...
        **kwargs,
        ...
        ), f"'{self.dataset_path}' directory does not exist. Please check the path or download the dataset following the instructions here: https://github.com/SoccerNet/sn-tracking"
...
def read_motchallenge_formatted_file(file_path):
    ...
    df["bbox_ltwh"] = df.apply(
        lambda row: np.array([row["left"], row["top"], row["width"], row["height"]]),
        axis=1,
    )
...
def load_set(dataset_path, nvid=-1, vids_filter_set=None):
    ...
    for video_folder in tqdm(
        sorted(video_list), desc=f"Loading SoccerNetGS '{split}' set videos"
    ):  # Sort videos by name
        ...
        if os.path.isdir(video_folder_path):
            ...
            for i in range(1, int(gameinfo_data.get("num_tracklets", 0)) + 1):
                ...
                if "goalkeeper" in role:
                    ...
                    role = "goalkeeper"
                    jersey_number = (
                        int(additional_info) if additional_info.isdigit() else None
                    )
                    position = None
                    category = (
            ...
            for t_id, t_attributes in tracklet_attributes.items():
                for attribute in t_attributes.keys():
                    detections_df.loc[detections_df["track_id"] == t_id, attribute] = (
