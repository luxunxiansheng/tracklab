...
class Normalize(torch.nn.Module):
    ...
    def forward(self, sample):
        ...
        return {"image": image, "data": sample["data"]}
...
class LRAmbiguityFix(torch.nn.Module):
    ...
    def forward(self, sample):
        ...
        if len(data) == 0:
            return {"image": sample["image"], "data": sample["data"]}
        ...
        if len(angles_h) > 0 and len(angles_v) > 0:
            if np.mean(angles_h) < self.h_th and np.mean(angles_v) < self.v_th:
                if n_right > n_left:
                    data = flip_annot_names(
                        data, swap_top_bottom=False, swap_posts=False
                    )

        return {"image": sample["image"], "data": data}
    ...
    def calculate_angle_v(self, x1, y1, x2, y2):
        if not x2 - x1 == 0:
            slope = (y2 - y1) / (x2 - x1)
            angle = (
                math.atan(1 / slope) if slope != 0 else math.pi / 2
...
no_transforms_w_LR = v2.Compose([ToTensor(), LRAmbiguityFix(v_th=70, h_th=20)])
