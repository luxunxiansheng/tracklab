    ...
    def process(self, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        xa_coordinates = [
            (
                ...
                if isinstance(bbox, dict) and bbox["x_bottom_middle"] is not None
                else np.nan
            )
        ...
        goalkeepers = detections[
            (detections.role == "goalkeeper")
            & detections.bbox_pitch.apply(
                lambda bbox: isinstance(bbox, dict)
                and bbox["x_bottom_middle"] is not None
            )
        ]
