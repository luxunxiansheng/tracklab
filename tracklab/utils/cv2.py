        ...
        for xy, c in zip(keypoints_xy, keypoints_c):
            ...
            if draw_skeleton:
                for link in posetrack_human_skeleton:
                    if (
                        (
                            keypoints_c[link[0] - 1] >= threshold
                            and keypoints_c[link[1] - 1] >= threshold
                        )
    ...
    if hasattr(detection, "bbox_ltwh"):
        ...
        if print_confidence:
            if hasattr(detection, "bbox_conf"):
                ...
                log.warning(
                    "No 'bbox_conf' found in the detection for confidence drawing."
                )
        if print_id:
            ...
            # Note: No warning logged when track_id is missing - this is expected in detection-only mode
...
def pretty_print(stat, value):
    if stat in ["hits", "age", "time_since_update"]:
        ...
        return {
            k: (
                {ik: round(100 * iv, 1) for ik, iv in v.items()}
...
def draw_text(
    ...
    color_txt=(
        ...
        0,
        0,
    ),  # RGB or None for automatic black or white if color_bg is not None
    ...
    text_sizes = [
        cv2.getTextSize(
            line, fontFace=fontFace, fontScale=fontScale, thickness=thickness
        )[0]
        for line in lines
    ]
    ...
    if color_bg is not None:
        ...
        crop = img[
            max(y_start, 0) : min(y_stop, img.shape[0]),
            max(x_start, 0) : min(x_stop, img.shape[1]),
        ]
        if crop.size > 0:
        ...
        if color_txt is None:
            color_txt = (
                np.array(distinctipy.get_text_color(np.array(color_bg) / 255)) * 255
            )
...
def colored_body_parts_overlay(
    img,
    masks,
    clip=True,
    ...
    for i in range(masks.shape[0]):
        ...
        white_bckg = (
            white_bckg * (1 - mask_alpha * weight) + color_img * mask_alpha * weight
        )
    ...
    masked_img = img * (1 - mask_alpha * alpha) + white_bckg * mask_alpha * alpha
    return masked_img
